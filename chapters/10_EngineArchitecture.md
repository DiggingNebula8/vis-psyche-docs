...
|---------|--------------|-----|
| Object doesn't move | Forgot to rebuild model matrix | Call `transform.GetModelMatrix()` after changes |
| Everything is huge/tiny | Wrong scale or projection | Check aspect ratio and `Transform::Scale` |
| Movement is jerky | Not using delta time | Multiply velocities by `deltaTime` |
| Object at wrong place | Order of transforms wrong | SRT order: Scale × Rotate × Translate |

---

## Checkpoint

This chapter covered engine architecture:

**Key Classes:**
| Class | Responsibility |
|-------|----------------|
| `Transform` | Position, rotation, scale → Model matrix |
| `Camera` | Eye position, target → View + Projection |
| `Mesh` | VAO, VBO, IBO + factory methods |
| `SceneObject` | Groups Mesh + Transform |
